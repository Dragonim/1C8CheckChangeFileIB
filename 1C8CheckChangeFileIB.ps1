# Описание: Скрипт позволяте убедиться что файловая ИБ не была изменена во время копирования
# Автор: Dim
# Версия: 1.0
...
param([switch]$Befor = $false, # установите данный флаг, если скрипт запускается до начала резервного копирования
      ...
      [string]$EmailPort = "587", # порт SMTP сервера (если порт отличен нот 25 то будет установлен тип шифрования SSL) [для яндекса и мейл.ру надо использовать порт 587]
      [string]$EmailLogin = "test@mail.ru", # Логин для доступа к электронной почте для отпарвки сообщения
      ...
      [int]$LockTime = 300, # время в секунда на которое будет заблокирована база, если ноль, то блокировка перманентная
      [int]$HashsumTimeout = 60, # время в секунда между созданием файла блокировки и получением хеш-суммы файла. За это время платформа должна успеть выбросить всех пользователей из ИБ
...
# добавляем в конец пути обратный слешь, если такого там нет
...
function GetHash($File) {
    ...
    # к сожалению Get-FileHash не может получить хэш используемого файла, поэтому необходимо пользоваться внешней программой    
    # возможно программа для получения хэш суммы была скачена ранее, проверим это
